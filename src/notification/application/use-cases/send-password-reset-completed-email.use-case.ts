...
export class SendPasswordResetCompletedEmailUseCase {
  private readonly logger = new Logger(SendPasswordResetCompletedEmailUseCase.name);
  ...
  async execute(input: SendPasswordResetCompletedEmailInput): Promise<void> {
    if (!input.email) {
      this.logger.warn('SendPasswordResetCompletedEmailUseCase: email is missing, skipping');
      return;
    }
    ...
    try {
      ...
      const html = await this.templateRenderer.render(NotificationTemplate.PASSWORD_RESET_COMPLETED, {
        appName,
        appDomain,
        email: input.email,
      ...
      this.logger.error(`Failed to send password reset completed email to ${input.email}`, error);
    }
  }
}
