...
import { mockCredentials, mockPasswordResetToken } from '../../__fixtures__/auth.fixtures';
...
describe('ConfirmPasswordResetUseCase', () => {
  ...
  const inputConfirm = { token: 'raw-reset-token-value', newPassword: 'NewStrongPass99!' };
  ...
  describe('execute', () => {
    it('resets password, invalidates sessions, and publishes event on happy path', async () => {
      ...
      mockPasswordResetTokenRepo.findByHash.mockResolvedValue(expectedResetToken);
      ...
      expect(mockPasswordResetTokenRepo.deleteById).toHaveBeenCalledWith(expectedResetToken.id);
      expect(mockRefreshTokenRepo.deleteAllByCredentialsId).toHaveBeenCalledWith(expectedCredentials.id);
      expect(mockEventBus.publish).toHaveBeenCalledWith(expect.any(UserPasswordResetCompletedEvent));
    });

    it('looks up the token by SHA-256 hash of the raw input token', async () => {
      const expectedHash = createHash('sha256').update(inputConfirm.token).digest('hex');
      ...
      expect(mockPasswordResetTokenRepo.findByHash).toHaveBeenCalledWith(expectedHash);
    });
    ...
    it('deletes the expired token and throws RESET_TOKEN_EXPIRED', async () => {
      const inputExpiredToken = mockPasswordResetToken({ expiresAt: new Date('2000-01-01T00:00:00.000Z') });
      mockPasswordResetTokenRepo.findByHash.mockResolvedValue(inputExpiredToken);
      mockPasswordResetTokenRepo.deleteById.mockResolvedValue(undefined);

      await expect(useCase.execute(inputConfirm)).rejects.toMatchObject({
        code: AuthenticationErrorCode.RESET_TOKEN_EXPIRED,
      });
      expect(mockPasswordResetTokenRepo.deleteById).toHaveBeenCalledWith(inputExpiredToken.id);
      expect(mockPasswordHasher.hash).not.toHaveBeenCalled();
    });
    ...
    it('throws AppError for all error cases', async () => {
      mockPasswordResetTokenRepo.findByHash.mockResolvedValue(null);
      await expect(useCase.execute(inputConfirm)).rejects.toBeInstanceOf(AppError);
    });
  });
});
