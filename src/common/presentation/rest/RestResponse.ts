...
export default class RestResponse<T> {
  ...
  constructor(message: string, error?: RestError, data?: T) {
  ...
  static error(
    ...
    const error: RestError = { name, message: errorMessage };
    ...
    return new HttpException(
      new RestResponse(message, error, undefined),
      status,
    );
  }
